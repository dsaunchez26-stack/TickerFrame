...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      { term: 'Volume (options)', def: 'How many contracts of this specific strike/expiration traded today - the main liquidity signal used here. Open Interest (contracts currently outstanding) isn\'t shown: the data provider this site uses doesn\'t report it, so rather than estimate a number that can\'t be verified, it\'s left out entirely.' },
      { term: 'Delta / Gamma / IV (how they\'re calculated)', def: 'This site\'s data provider supplies real bid/ask/last/volume for every contract but not greeks or implied volatility, so delta, gamma, and IV are computed here directly from that real price data using the standard Black-Scholes formula - the same math any provider\'s "greeks" field is built from, not an estimate or a fabricated number.' },
